...
//
// Музыкальное время считается в сэмплах звукового вывода, в задаче звука
// на ядре 0. Поэтому отрисовка экрана на ядре 1 не может сдвинуть долю.
// Блок рендера обрывается ровно на границе шага, и удар попадает на свой
// сэмпл — так же, как в офлайн-рендере экспорта.
//
// Разделение ролей:
...
class Transport : public AudioSource {
 ...
 private:
  ...
  // Длина очередного шага в сэмплах. Шаг почти никогда не равен целому
  // числу сэмплов: на 137 BPM и 16 кГц это 7007,30. Если просто
  // округлять вниз, сетка уезжает на ~2,6 мс в минуту. Поэтому остаток от
  // деления копится и раз в несколько шагов добавляет лишний сэмпл.
  // Граница шага k встаёт ровно в floor(k * rate * 60 / (bpm *
  // kStepsPerBeat)) — туда же, куда её ставит экспорт.
