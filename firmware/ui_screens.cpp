...
// Главный экран: слева BPM/метроном/разделы проекта в колонке
...
void UiScreens::showOff() {
  ...
  drawText((kScreenW - (int16_t)strlen(hint) * 6) / 2, 196, hint, 1, kHint, ILI9341_BLACK);
}
...
void UiScreens::drawHeader(const char* rightLabel, uint16_t rightColor) {
  drawText(20, 24, "SMPLR", 2, kCream, kBg);

  int16_t textW = (int16_t)strlen(rightLabel) * 6;  // 5x7 font, size 1
  drawText(kScreenW - 20 - textW, 30, rightLabel, 1, rightColor, kBg);

  tft_.drawFastHLine(20, 46, kScreenW - 40, kDim);
}

void UiScreens::drawBootFooter() {
  drawText(21, kFooterY, "PRERELEASE V" SMPLR_VERSION, 1, kMagenta, kBg);

  const char* right = "SMPLR OS";
  const int16_t textW = (int16_t)strlen(right) * 6;
  drawText(kScreenW - 21 - textW, kFooterY, right, 1, kCyan, kBg);
}
...
void UiScreens::showBoot(uint8_t percent) {
  if (!bootDrawn_) {
    ...
    drawLogoRows(0, kCream, 0, kSmplrLogoHeight);
    drawBootFooter();
    for (uint8_t i = 0; i < kBootBarSegments; i++) {
      const int16_t x = kBarX + i * (kBarSegW + kBarGap);
      tft_.fillRect(x, kBarY, kBarSegW, kBarSegH, kGlitchDim);
    }
    lastFilledSegs_ = 0;
    lastBootPercent_ = kNotDrawn;
    lastDotPhase_ = kNotDrawn;
...
// Верхняя строка: крупный BPM и кнопка метронома рядом.
constexpr int16_t kHomeTopRowY = 51;
constexpr int16_t kHomeTopRowH = 46;
constexpr int16_t kHomeBpmBoxX = kHomeBpmX - 10;  // область BPM вместе с рамкой фокуса
constexpr int16_t kHomeBpmBoxW = 92;              // три цифры + отступы
constexpr int16_t kHomeBpmTextY = kHomeTopRowY + 7;
constexpr int16_t kHomeMetY = kHomeTopRowY;
constexpr int16_t kHomeMetW = 56;
constexpr int16_t kHomeMetH = kHomeTopRowH;
...
void UiScreens::drawHomeBpm(uint16_t bpm, bool focused) {
  tft_.fillRect(kHomeBpmBoxX, kHomeTopRowY, kHomeBpmBoxW, kHomeTopRowH, kBg);

  char bpmStr[6];
  snprintf(bpmStr, sizeof(bpmStr), "%u", bpm);
  const int16_t bpmTextW = (int16_t)strlen(bpmStr) * 24;  // 6px*size4/char
  drawText(kHomeBpmX, kHomeBpmTextY, bpmStr, 4, kCream, kBg);

  if (focused) {
    const int16_t frameW = max(bpmTextW, (int16_t)48) + 20;
    // Рамка в два пикселя: внешний контур и внутренний на 1 px глубже.
    tft_.drawRoundRect(kHomeBpmBoxX, kHomeTopRowY, frameW, kHomeTopRowH, 6, kOrange);
    tft_.drawRoundRect(kHomeBpmBoxX + 1, kHomeTopRowY + 1, frameW - 2, kHomeTopRowH - 2, 5,
                       kOrange);
  }

  // Подпись под темпом говорит, чем его крутить: K1 меняет темп всегда,
  // курсор на BPM только подсвечивает подсказку.
  tft_.fillRect(kHomeBpmX, kHomeBpmLabelY, 90, 8, kBg);
  drawText(kHomeBpmX, kHomeBpmLabelY, "BPM", 1, kOrange, kBg);
  drawText(kHomeBpmX + 24, kHomeBpmLabelY, focused ? "TURN K1" : "K1", 1,
           focused ? kCream : kHint, kBg);
}

// Кнопка метронома — справа от темпа, в той же строке.
void UiScreens::drawHomeMetronome(bool on, bool focused) {
  tft_.fillRect(kHomeMetX - 3, kHomeMetY - 3, kHomeMetW + 6, kHomeMetH + 6, kBg);
  uint16_t fg;
  uint16_t bg;
  if (on) {
    tft_.fillRoundRect(kHomeMetX, kHomeMetY, kHomeMetW, kHomeMetH, 6, kGreen);
    fg = kBg;
    bg = kGreen;
  } else {
    tft_.drawRoundRect(kHomeMetX, kHomeMetY, kHomeMetW, kHomeMetH, 6, kDim);
    fg = kHint;
    bg = kBg;
  }
...
void UiScreens::drawHomeSection(uint8_t section, bool active, bool cursor) {
  ...
  tft_.fillRect(x - 3, kHomeRowY - 3, kHomeBoxW + 6, 38, kBg);
  uint16_t fg;
  uint16_t bg;
  if (active) {
    tft_.fillRoundRect(x, kHomeRowY, kHomeBoxW, 32, 5, kOrange);
    fg = kBg;
    bg = kOrange;
  } else {
    tft_.drawRoundRect(x, kHomeRowY, kHomeBoxW, 32, 5, kDim);
    fg = kHint;
    bg = kBg;
  }
  ...
  if (cursor) {
    tft_.drawRoundRect(x - 3, kHomeRowY - 3, kHomeBoxW + 6, 38, 7, kCream);
  }
  ...
  tft_.fillRect(x - 3, kHomeSectionLabelY, kHomeBoxW + 6, 8, kBg);
  drawText(x + (kHomeBoxW - nameW) / 2, kHomeSectionLabelY, name, 1,
           active ? kOrange : kHint, kBg);
}
...
void UiScreens::drawHomeStatus(const HomeView& v) {
  ...
  tft_.fillRect(kHomeStatusRight - kHomeStatusW, 26, kHomeStatusW, 12, kBg);
  const uint16_t color = v.playing && v.silent ? kHint : kGreen;
  drawText(kHomeStatusRight - textW, 30, label, 1, color, kBg);
  if (v.playing) {
...
void UiScreens::drawHomeExportBadge(const HomeView& v) {
  ...
  tft_.fillRect(kHomeExportX, 26, kHomeExportW, 12, kBg);
  if (label[0] != '\0') drawText(kHomeExportX, 30, label, 1, color, kBg);
}
...
void UiScreens::showHome(const HomeView& v) {
  ...
  if (screen_ == Screen::Home) {
    ...
    drawHomeBpm(v.bpm, v.focus == HomeFocus::Bpm);
    drawHomeMetronome(v.metronomeOn, v.focus == HomeFocus::Metronome);
    drawText(kHomeMetX + 9, kHomeBpmLabelY, "PAD4", 1, kHint, kBg);
    for (uint8_t i = 0; i < kSectionCount; i++) {
      drawHomeSection(i, i == v.activeSection, i == cursor);
    }

    tft_.drawFastHLine(20, kHomeHintLineY, kHomeLeftW - 20, kDim);
    drawText(20, kHomeHintLineY + 6, "PAD1/2/3/6 MOVE  PAD5 SELECT,", 1, kHint, kBg);
    drawText(20, kHomeHintLineY + 16, "AGAIN TO OPEN  MODE MENU", 1, kHint, kBg);
    ...
    tft_.drawRoundRect(kMeterX - 4, kMeterY - 4, kMeterW + 8, kMeterH + 8, 4, kDim);
    drawText(kMeterX + (kMeterW - 18) / 2, kMeterBottom + 8, "OUT", 1, kHint, kBg);
    lastMeterFillPx_ = kNotDrawn;
...
void UiScreens::updateSoundMeter(float level) {
  ...
  if (lastMeterFillPx_ == kNotDrawn) {
    tft_.fillRect(kMeterX, kMeterY, kMeterW, kMeterH - fillPx, kBg);
    if (fillPx > 0) {
      tft_.fillRect(kMeterX, kMeterY + (kMeterH - fillPx), kMeterW, fillPx, kOrange);
    }
...
void UiScreens::drawPageFrame(const char* title, const char* hint1, const char* hint2) {
  tft_.fillScreen(kBg);
  screen_ = Screen::Other;
  drawHeader(title, kOrange);
  tft_.drawFastHLine(20, kPageFooterLineY, kScreenW - 40, kDim);
  if (hint1 != nullptr) drawText(20, kPageHint1Y, hint1, 1, kHint, kBg);
  if (hint2 != nullptr) drawText(20, kPageHint2Y, hint2, 1, kHint, kBg);
}
...
void UiScreens::drawMenuRow(uint8_t item, bool selected) {
  ...
  uint16_t bg;
  if (selected) {
    tft_.fillRoundRect(20, y, kScreenW - 40, rowH - 6, 5, kOrange);
    fg = kBg;
    bg = kOrange;
  } else {
    tft_.fillRect(20, y, kScreenW - 40, rowH - 6, kBg);
    tft_.drawRoundRect(20, y, kScreenW - 40, rowH - 6, 5, kDim);
    fg = kCream;
    bg = kBg;
  }
...
void UiScreens::showMenuStub(uint8_t itemIndex) {
  ...
  drawText(20, 64, "COMING SOON", 2, kCream, kBg);
  if (itemIndex == kMenuItemTrack) {
    drawText(20, 96, "SAMPLE PER CHANNEL - NEEDS SD CARD", 1, kHint, kBg);
    drawText(20, 110, "CHANNEL VOLUME AND MUTE ARE IN", 1, kHint, kBg);
    drawText(20, 122, "SECTION 2 - MIX", 1, kHint, kBg);
  }
}

void UiScreens::drawTempoRow(uint8_t row, uint16_t bpm, bool metronomeOn, bool selected) {
  const int16_t y = kTempoRowY[row];
  tft_.fillRect(kTempoRowX, y, kTempoRowW, kTempoRowH, kBg);
  tft_.drawRoundRect(kTempoRowX, y, kTempoRowW, kTempoRowH, 5,
                     selected ? kOrange : kDim);
  if (selected) {
    tft_.drawRoundRect(kTempoRowX + 1, y + 1, kTempoRowW - 2, kTempoRowH - 2, 4, kOrange);
  }
  char value[8];
  uint16_t valueColor = kCream;
  if (row == 0) {
    drawText(kTempoRowX + 12, y + 13, "BPM", 1, kHint, kBg);
    snprintf(value, sizeof(value), "%u", bpm);
  } else {
    drawText(kTempoRowX + 12, y + 13, "METRONOME", 1, kHint, kBg);
    snprintf(value, sizeof(value), "%s", metronomeOn ? "ON" : "OFF");
    valueColor = metronomeOn ? kGreen : kHint;
  }
  const int16_t w = (int16_t)strlen(value) * 12;
  drawText(kTempoRowX + kTempoRowW - 16 - w, y + 10, value, 2, valueColor, kBg);
}
...
void UiScreens::showTempo(uint16_t bpm, bool metronomeOn, uint8_t row) {
  if (screen_ != Screen::Tempo) {
    ...
    drawText(20, 160, "METRONOME CLICKS ONLY WHILE PLAYING,", 1, kHint, kBg);
    drawText(20, 172, "ON EVERY BEAT, ACCENT ON BAR START", 1, kHint, kBg);
  } else {
...
void UiScreens::drawInputValues(uint8_t knobSpeed, uint8_t lastKnob, int8_t lastDelta) {
  ...
  drawTextField(200, 62, 100, buf, 2, kCream, kBg);
  ...
  drawTextField(200, 136, 100, buf, 2, kCream, kBg);
}
...
void UiScreens::showInput(uint8_t knobSpeed, uint8_t lastKnob, int8_t lastDelta) {
  if (screen_ != Screen::Input) {
    ...
    drawText(20, 66, "KNOB SPEED", 1, kHint, kBg);
    drawText(20, 84, "HOW MUCH ONE KNOB CLICK CHANGES", 1, kHint, kBg);
    drawText(20, 96, "A VALUE (BPM, VOLUME)", 1, kHint, kBg);
    drawText(20, 140, "LAST KNOB", 1, kHint, kBg);
    drawText(20, 166, "SOURCE  WOKWI VIRTUAL MPK", 1, kHint, kBg);
    drawText(20, 178, "KNOBS ARE ENDLESS: VALUES NEVER JUMP", 1, kHint, kBg);
...
void UiScreens::showExport(const ExportView& v) {
  ...
  drawText(20, 96, info, 1, kHint, kBg);

  const char* statusText = "";
  uint16_t statusColor = kCream;
  switch (v.status) {
  ...
  drawText(20, 120, statusText, 2, statusColor, kBg);

  tft_.drawRect(kExportBarX - 2, kExportBarY - 2, kExportBarW + 4, kExportBarH + 4, kDim);
  ...
  switch (v.status) {
    case ExportStatus::Running:
      drawText(20, 172, "PLAY/STOP IS OFF WHILE EXPORTING", 1, kHint, kBg);
      break;
    case ExportStatus::Done:
      drawText(20, 172, v.fileName, 1, kCream, kBg);
      drawText(20, 186, "SAVE WITH THE SMPLR EXPORT BOOKMARK", 1, kHint, kBg);
      break;
    case ExportStatus::Aborted:
      drawText(20, 172, "NO FILE WAS SAVED", 1, kHint, kBg);
      break;
    case ExportStatus::Empty:
      drawText(20, 172, "DRAW SOME STEPS IN SECTION 1 (SEQ)", 1, kHint, kBg);
      break;
    default:
      drawText(20, 172, "PLAYBACK STOPS WHILE EXPORTING", 1, kHint, kBg);
      break;
  }
}
...
void UiScreens::updateExportProgress(uint8_t percent) {
  ...
  tft_.fillRect(kExportBarX, kExportBarY, fillW, kExportBarH, kOrange);
  tft_.fillRect(kExportBarX + fillW, kExportBarY, kExportBarW - fillW, kExportBarH, kBg);

  char pct[6];
  snprintf(pct, sizeof(pct), "%u%%", (unsigned)percent);
  drawTextField(kScreenW - 20 - 24, 126, 24, "", 1, kCream, kBg);
  drawText(kScreenW - 20 - (int16_t)strlen(pct) * 6, 126, pct, 1, kCream, kBg);
}
...
void UiScreens::drawSequencerCell(const StepSequencer& seq, uint8_t track, uint8_t step,
  ...
  tft_.fillRect(x - 2, y - 2, kSeqCellW + 4, kSeqCellH + 4, kBg);
  if (on) {
    // Сработавший шаг под бегущей полосой вспыхивает кремовым.
    tft_.fillRect(x, y, kSeqCellW, kSeqCellH, playhead ? kCream : uicolor::kTrack[track]);
  } else if (playhead) {
  ...
  if (cursor) {
    tft_.drawRect(x - 2, y - 2, kSeqCellW + 4, kSeqCellH + 4, kCream);
    tft_.drawRect(x - 1, y - 1, kSeqCellW + 2, kSeqCellH + 2, kCream);
  }
}
...
void UiScreens::drawSequencerPlayheadColumn(const StepSequencer& seq, uint8_t step, bool lit,
  ...
  const int16_t x = seqStepX(step);
  tft_.fillRect(x, kSeqMarkerY, kSeqCellW, kSeqMarkerH, lit ? kCream : kBg);
  for (uint8_t t = 0; t < StepSequencer::kTracks; t++) {
    ...
    if (lit) {
      tft_.fillCircle(8, seqTrackY(t) + kSeqCellH / 2, 3,
                      seq.isOn(t, step) ? uicolor::kTrack[t] : kBg);
    }
  }
}
...
void UiScreens::showSequencer(const StepSequencer& seq, bool playing, uint8_t playhead,
  ...
  drawText(12, 14, "STEP SEQ", 2, kCream, kBg);
  ...
  seqPlayheadStep_ = playing ? playhead : StepSequencer::kNoStep;
  for (uint8_t t = 0; t < StepSequencer::kTracks; t++) {
  ...
  if (seqPlayheadStep_ != StepSequencer::kNoStep) {
    drawSequencerPlayheadColumn(seq, seqPlayheadStep_, true, cursorTrack, cursorStep);
  }

  tft_.drawFastHLine(12, kPageFooterLineY, kScreenW - 24, kDim);
  drawText(12, kPageHint1Y, "PAD1/2/3/6 MOVE  PAD5 STEP  PAD7 BACK", 1, kHint, kBg);
  updateSequencerFooter(StepSequencer::kNoTrack);
}

void UiScreens::updateSequencerFooter(uint8_t confirmTrack) {
  char text[40];
  uint16_t color = kHint;
  ...
  drawTextField(12, kPageHint2Y, kScreenW - 24, text, 1, color, kBg);
}
...
void UiScreens::updateSequencerPlayhead(const StepSequencer& seq, uint8_t playhead,
  ...
  if (next == StepSequencer::kNoStep) {
    for (uint8_t t = 0; t < StepSequencer::kTracks; t++) {
      tft_.fillCircle(8, seqTrackY(t) + kSeqCellH / 2, 3, kBg);
    }
  } else {
    ...
    for (uint8_t t = 0; t < StepSequencer::kTracks; t++) {
      tft_.fillCircle(8, seqTrackY(t) + kSeqCellH / 2, 3,
                      seq.isOn(t, next) ? uicolor::kTrack[t] : kBg);
    }
  }
}

void UiScreens::updateSequencerHeader(bool playing, uint16_t bpm, bool metronomeOn) {
  tft_.fillRect(kSeqTransportX, 10, kScreenW - 12 - kSeqTransportX, 24, kBg);

  const int16_t iconX = kSeqTransportX + 4;
  const int16_t iconY = 14;
  if (playing) {
    tft_.fillTriangle(iconX, iconY, iconX, iconY + 14, iconX + 12, iconY + 7, kGreen);
  } else {
    tft_.fillRect(iconX, iconY + 1, 12, 12, kHint);
  }
  drawText(iconX + 18, iconY + 4, playing ? "PLAY" : "STOP", 1,
           playing ? kGreen : kHint, kBg);

  // Метроном виден и здесь: PAD4 включает его с любого экрана.
  if (metronomeOn) {
    tft_.fillRoundRect(kSeqMetX, iconY, 26, 14, 3, kGreen);
    drawText(kSeqMetX + 4, iconY + 4, "MET", 1, kBg, kGreen);
  } else {
    tft_.drawRoundRect(kSeqMetX, iconY, 26, 14, 3, kDim);
    drawText(kSeqMetX + 4, iconY + 4, "MET", 1, kHint, kBg);
  }

  char bpmStr[10];
  snprintf(bpmStr, sizeof(bpmStr), "%u BPM", bpm);
  drawText(kScreenW - 12 - (int16_t)strlen(bpmStr) * 6, iconY + 4, bpmStr, 1, kCream,
           kBg);
}
