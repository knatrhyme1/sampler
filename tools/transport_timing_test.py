...
Наивное `длина_шага = rate * 60 / (bpm * 2)` с округлением вниз этого НЕ даёт:
на 137 BPM шаг равен 3503,65 сэмпла, и накопленное округление уводит сетку на
10 мс меньше чем за минуту. Поэтому в коде остаток от деления копится
