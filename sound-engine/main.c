/* Стори 0.3: звуковой движок, отдельный от прошивки, собирается на
 * компьютере без ESP32. Пока умеет одно: записать 1 секунду тона
 * 440 Гц в WAV-файл. Дальше сюда ляжет секвенсор и сэмплер (эпик 4). */
